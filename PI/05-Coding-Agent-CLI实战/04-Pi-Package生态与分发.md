...
对于用官方安装器（installer）管理的 pi 安装（目前是实验特性），`pi update --self` 走的是"先在一个带 lockfile 的暂存目录里装好新版本、校验通过后再原子切换为当前发行版"的流程，如果更新过程中校验失败，会保留原来那个仍然可用的版本不受影响；这类受管安装不支持 `--force`，出问题时需要重新运行一遍安装器来修复，而不是加 `--force` 强推。
...
路径相对于包根目录解析，数组支持 glob 通配和 `!排除项`。两个容易踩的细节：manifest 里的正向 glob 只按字典序发现"可见"路径,点开头（`.`）的隐藏路径不会被 glob 匹配到,必须显式列出;如果某个资源目录本身是通过符号链接挂进来的,glob 不会穿过符号链接继续遍历,同样需要把这个符号链接指向的资源根目录直接列出来,而不是依赖上层目录的通配符。
